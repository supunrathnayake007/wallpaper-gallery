...
export async function getWallpapersThumb(
  ...
  try {
    // Ensure downloadedIds is an array or default to an empty array if null
    const validDownloadedIds = Array.isArray(downloadedIds)
      ? downloadedIds
      : downloadedIds
      ? [downloadedIds]
      : [];
    ...
    const pipeline = [
      ...
      {
        $match: {
          _id: { $nin: validDownloadedIds.map((id) => new ObjectId(id)) },
        },
      },
...
export async function getFieldByIds(collection, field_name, ids) {
  try {
    console.log(" getFieldByIds function runs ... ");
    ...
    console.log("catch from mongodb.js|getFieldsByIds|catch -" + error.message);
    await client.close();
    return { error };
  }
}
